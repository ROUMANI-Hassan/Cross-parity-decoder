...
# Objectif : remplir FIFO, lancer Decod, observer FSM, buffer, analyse,
#            correction, status, FIFO sortie et donnees lues.
# ================================================================
...
add wave -divider "===== FIFO ENTREE : DONNEES RECUES PAR LE DPC ====="
...
add wave -divider "===== BUFFER 8x8 : MATRICE CHARGEE DEPUIS FIFO ====="
...
add wave -divider "===== ANALYSE PARITE : VECTEURS RECUS/CALCULES ====="
...
add wave -divider "===== FIFO SORTIE : DONNEES RENVOYEES AU SYSTEME ====="
...
proc write_matrix {m0 m1 m2 m3 m4 m5 m6 m7} {
    # Addr=10 correspond a DPCDataIn / DPCDataOut
...
proc launch_decode {} {
    # DPCControl : bit0 Decod=1, bit5 IrqEn=1 => 0x21
    write_reg 01 21 "DPCControl : IrqEn=1, Decod=1"
    run 900 ns
    read_reg 00 "lecture DPCStatus : efface Done/Corr/Error apres lecture"
    run 120 ns
}

proc read_output_fifo {} {
    echo "Lecture des 7 mots utiles dans DPCDataOut"
...
# ================================================================
# MATRICE DE REFERENCE ENCODEE 8x8
# Donnees utiles 7x7 + bit de parite ligne en bit7.
# Derniere ligne = parites colonnes bits[6:0] + parite globale bit7.
# Reference sans erreur : 55 33 71 8E CC AA 17 68
# ================================================================

# ------------------------------------------------
# TEST 1 : aucune erreur => accept_raw=1, Corr=0, Error=0, Done=1
# ------------------------------------------------
reset_dut
add wave -divider "===== TEST 1 : MATRICE SANS ERREUR ====="
echo "\n================ TEST 1 : SANS ERREUR ================"
...
# ------------------------------------------------
# TEST 2 : une erreur dans donnees utiles
# Exemple : ligne 2, colonne 3 : 0x71 xor 0x08 = 0x79
# Attendu : corr_en=1, Corr=1, data sortie corrigee = ligne 2 redevient 0x71 utile.
# ------------------------------------------------
#reset_dut
#add wave -divider "===== TEST 2 : 1 ERREUR UTILE CORRIGEE ====="
#echo "\n================ TEST 2 : 1 ERREUR UTILE ================"
#write_matrix 55 33 79 8E CC AA 17 68
#launch_decode
#read_output_fifo

# ------------------------------------------------
# TEST 3 : une erreur sur un bit de parite ligne
# Exemple : ligne 3 bit7 : 0x8E xor 0x80 = 0x0E
# Attendu : accept_raw=1, Corr=0, Error=0, les donnees utiles sont envoyees.
# ------------------------------------------------
#reset_dut
#add wave -divider "===== TEST 3 : 1 ERREUR PARITE, DONNEES ACCEPTEES ====="
#echo "\n================ TEST 3 : 1 ERREUR PARITE ================"
#write_matrix 55 33 71 0E CC AA 17 68
#launch_decode
#read_output_fifo

# ------------------------------------------------
# TEST 4 : deux erreurs utiles => err_en=1, Error=1, pas de donnees envoyees
# Exemple : ligne 1 col0 et ligne 2 col3.
# ------------------------------------------------

#reset_dut
#add wave -divider "===== TEST 4 : ERREUR NON CORRIGEABLE ====="
#echo "\n================ TEST 4 : 2 ERREURS UTILES ================"
#write_matrix 55 32 79 8E CC AA 17 68
#launch_decode
#read_reg 10 "tentative lecture DPCDataOut : FIFO sortie doit rester vide"

#wave zoom full
#run 200 ns

#echo "\nFIN DES TESTS. Observe surtout : state, buffer_8x8, vpl/vpc, err_l/err_c/err_g, accept_raw/corr_en/err_en, dpc_status, u_fifo_out/Fifo."

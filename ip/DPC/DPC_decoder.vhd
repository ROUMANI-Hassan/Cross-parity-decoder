...
-- Décodeur de Parité Croisée 7x7 + parités (matrice reçue 8x8)
-- Interface registres Avalon simple :
--   Addr = 0 : DPCStatus  (lecture)
--   Addr = 1 : DPCControl (lecture/écriture)
--   Addr = 2 : DPCDataIn  (écriture) / DPCDataOut (lecture)
  ...
  begin
    if Reset_n = '0' then
      ...
      case state is
        ...
        when S_LOAD_BUFFER =>
          -- DataOut de la FIFO est toujours visible : on capture la tête puis RdFifo décale.
          if load_cnt < 8 then
        ...
        when S_LOAD_DATA =>
          -- On ne renvoie que les 7 lignes de données utiles.
          if write_cnt < 7 then
        ...
        when S_DONE =>
          ...
          dpc_control(C_CTRL_DECOD) <= '0'; -- fin de traitement : Decod remis à 0
